...
async function executionFixture(mode, names = ['entrada.js'], executionId = 'exec-restore') {
  ...
  const response = await runBridgeRequest({ command: 'execute', confirmed: true, authorizeOverwriteConflicts: true, executionId }, { projectRoot: root });
...
test('.min remove somente saída criada; saída preexistente permanece', async () => {
  ...
  try {
    ...
    const execution = await runBridgeRequest({ command: 'execute', confirmed: true, authorizeOverwriteConflicts: true, executionId: 'exec-min' }, { projectRoot: root });
